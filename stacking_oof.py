...
"""
Generic Out-of-Fold (OOF) Stacking Framework

Usage
-----
python oof_stacking.py \
...
def create_transformer(input_shape,
    ...
    ffn = Dense(embed_dim,
                kernel_initializer=initializer)(ffn)
...
def generate_oof_predictions(
    ...
    for fold_id, (tr_idx, val_idx) in enumerate(
        ...
        test_preds_folds[fold_id] = (
            model.predict(X_test, verbose=0)
            .flatten()
        )

    return oof_preds, test_preds_folds.mean(axis=0)
...
def reshape_features(X, alphabet_size):
    ...
    if X.shape[1] % alphabet_size != 0:
        raise ValueError(
            "Feature dimension must be divisible by alphabet_size."
        )

    seq_length = X.shape[1] // alphabet_size

    return X.reshape(
        (-1, seq_length, alphabet_size)
    )


def run_oof_stacking_experiment(
    X_train,
    y_train,
    X_test,
    y_test,
    ...
    for seed in range(n_repeats):
        ...
        meta_X_train = np.column_stack(oof_features)
        meta_X_test = np.column_stack(test_features)
